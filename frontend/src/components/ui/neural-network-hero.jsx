...
export default function Hero({
  ...
  badgeLabel = "New",
  ctaButtons = [
    { text: "Get started", onClick: () => {}, primary: true },
    { text: "View showcase", onClick: () => {} }
  ],
  ...
  return (
    <section ref={sectionRef} className="relative h-screen w-screen overflow-hidden">
      ...
      <div className="relative mx-auto flex max-w-7xl flex-col items-start gap-6 px-6 pb-24 pt-36 sm:gap-8 sm:pt-44 md:px-10 lg:px-16">
        <div ref={badgeRef} className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1.5 backdrop-blur-sm opacity-0 animate-fade-in-down">
          <span className="text-[10px] font-light uppercase tracking-[0.08em] text-white/70">{badgeLabel}</span>
          <span className="h-1 w-1 rounded-full bg-white/40" />
          <span className="text-xs font-light tracking-tight text-white/80">{badgeText}</span>
        </div>

        <h1 ref={headerRef} className="max-w-2xl text-left text-5xl font-extralight leading-[1.05] tracking-tight text-white sm:text-6xl md:text-7xl opacity-0 animate-fade-in-up animation-delay-200">
          {title}
        </h1>

        <p ref={paraRef} className="max-w-xl text-left text-base font-light leading-relaxed tracking-tight text-white/75 sm:text-lg opacity-0 animate-fade-in-up animation-delay-400">
          {description}
        </p>

        <div ref={ctaRef} className="flex flex-wrap items-center gap-3 pt-2 opacity-0 animate-fade-in-up animation-delay-600">
        ...
        <ul ref={microRef} className="mt-8 flex flex-wrap gap-6 text-xs font-extralight tracking-tight text-white/60">
          {microDetails.map((detail, index) => {
            ...
            return (
              <li key={index} ref={refMap[index]} className="flex items-center gap-2 opacity-0 animate-fade-in-up" style={{ animationDelay: `${0.8 + index * 0.1}s` }}>
